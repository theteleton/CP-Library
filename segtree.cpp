...
T segmentTree<T>::query(node<T> *current_node, int range_left, int range_right, int left_index, int right_index) {
    ...
    return segmentTree<T>::query(current_node->left, range_left, range_right, left_index, (left_index + right_index) / 2) + segmentTree<T>::query(current_node->right, range_left, range_right, (left_index + right_index) / 2 + 1, right_index);
};
...
int main() {
    ...
    auto *obj = new segmentTree<long long>(root, &v);
